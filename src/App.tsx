...
const StatsView = ({ count, isDarkMode }: { count: number, isDarkMode: boolean }) => {
  return (
    <div className="max-w-4xl mx-auto py-12 px-4">
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className={`p-10 rounded-[2.5rem] border text-center ${isDarkMode ? 'bg-[#141414] border-white/10' : 'bg-white border-black/5 shadow-xl'}`}
      >
        ...
        <h2 className="text-4xl font-bold mb-4 tracking-tight">Processed Files</h2>
        <p className={`text-lg mb-10 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
          Total number of metadata and prompts generated by you.
        </p>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className={`p-8 rounded-3xl border ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-gray-50 border-black/5'}`}>
            <p className={`text-sm font-bold uppercase tracking-widest mb-2 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>Total Generated</p>
            <p className="text-6xl font-black text-indigo-500">{count}</p>
          </div>
          <div className={`p-8 rounded-3xl border ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-gray-50 border-black/5'}`}>
            <p className={`text-sm font-bold uppercase tracking-widest mb-2 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>Session Status</p>
            <p className="text-2xl font-bold text-emerald-400 mt-4">Active & Local</p>
            <p className="text-xs text-gray-500 mt-2 italic">Data saved to your local browser storage</p>
          </div>
        </div>
      </motion.div>
    </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {files.length === 0 && (
          ...
          <AboutPage isDarkMode={isDarkMode} />
            ...
            {/* Footer */}
            <footer className={`py-12 border-t mt-auto ${isDarkMode ? 'border-white/10 text-gray-500' : 'border-black/5 text-gray-400'}`}>
